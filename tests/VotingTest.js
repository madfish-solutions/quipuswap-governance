...
describe("Voting test", async function () {
  ...
  describe("Testing entrypoint: add_vote", async function () {
    ...
    it("The voting period is over", async function () {
      await rejects(contract.methods.vote("3", "for", "unit").send(), (err) => {
        strictEqual(err.message, "The voting period is over");
        return true;
      });
    });
    it("Voting has not started yet", async function () {
      await rejects(contract.methods.vote("0", "for", "unit").send(), (err) => {
        strictEqual(err.message, "Voting has not started yet");
        return true;
      });
    });
    it("This proposal has been blocked by the administrator", async function () {
      await rejects(contract.methods.vote("2", "for", "unit").send(), (err) => {
        strictEqual(
          err.message,
          "This proposal has been blocked by the administrator"
        );
        return true;
      });
    });
