...
describe("Proposal test", async function () {
  ...
  describe("Testing entrypoint: new_proposal", async function () {
    it("Minimum voting period 3 days", async function () {
      await rejects(
        contract.methods.new_proposal(ipfs_link, forum_link, 2).send(),
        (err) => {
    ...
    it("Maxium voting period 30 days", async function () {
      await rejects(
        contract.methods.new_proposal(ipfs_link, forum_link, 31).send(),
        (err) => {
    ...
    it("Successful new proposal", async function () {
      await contract.methods.new_proposal(ipfs_link, forum_link, 3).send();
  ...
  describe("Testing entrypoint: New_deferred_proposal", async function () {
    it("Minimum voting period 3 days", async function () {
      await rejects(
        contract.methods
          .new_deferred_proposal(ipfs_link, forum_link, 2, 5)
          .send(),
        (err) => {
          strictEqual(err.message, "Minimum voting period 3 days");
          return true;
    ...
    it("Maxium voting period 30 days", async function () {
      await rejects(
        contract.methods
          .new_deferred_proposal(ipfs_link, forum_link, 31, 5)
          .send(),
        (err) => {
          strictEqual(err.message, "Maxium voting period 30 days");
          return true;
    ...
    it("Deferral can't be longer than 30 days", async function () {
      await rejects(
        contract.methods
          .new_deferred_proposal(ipfs_link, forum_link, 4, 31)
          .send(),
        (err) => {
    ...
    it("Successful new deferred proposal", async function () {
      await contract.methods
        .new_deferred_proposal(ipfs_link, forum_link, 3, 3)
        .send();
